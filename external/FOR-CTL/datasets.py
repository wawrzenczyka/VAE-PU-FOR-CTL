...
def load_dataset(name):
    if name in ['ILPD', 'parkinsons', 'SPECTF', 'fertility', 'madelon']:
        df = pd.read_csv(os.path.join('data', f'{name}.csv'))
    else:
        data = arff.loadarff(os.path.join('data', f'{name}.arff'))
        df = pd.DataFrame(data[0])
    
    if name == 'parkinsons':
        X = df.iloc[:, 1:].drop('status', 1)
        y = df['status']
    elif name == 'SPECTF':
        X = df.iloc[:, 1:]
        y = df.iloc[:, 0]
    elif name == 'gas-concentrations':
        X = df.iloc[:, :-1]
        y = df.iloc[:, -1]
    else:
        X = df.iloc[:, :-1]
